// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.26.0
// 	protoc        v3.6.1
// source: v1/catalog.proto

package v1

import (
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type ListBeerReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	PageNum  int64 `protobuf:"varint,1,opt,name=page_num,json=pageNum,proto3" json:"page_num,omitempty"`
	PageSize int64 `protobuf:"varint,2,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
}

func (x *ListBeerReq) Reset() {
	*x = ListBeerReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_v1_catalog_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListBeerReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListBeerReq) ProtoMessage() {}

func (x *ListBeerReq) ProtoReflect() protoreflect.Message {
	mi := &file_v1_catalog_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListBeerReq.ProtoReflect.Descriptor instead.
func (*ListBeerReq) Descriptor() ([]byte, []int) {
	return file_v1_catalog_proto_rawDescGZIP(), []int{0}
}

func (x *ListBeerReq) GetPageNum() int64 {
	if x != nil {
		return x.PageNum
	}
	return 0
}

func (x *ListBeerReq) GetPageSize() int64 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

type ListBeerReply struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Results []*ListBeerReply_Beer `protobuf:"bytes,1,rep,name=results,proto3" json:"results,omitempty"`
}

func (x *ListBeerReply) Reset() {
	*x = ListBeerReply{}
	if protoimpl.UnsafeEnabled {
		mi := &file_v1_catalog_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListBeerReply) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListBeerReply) ProtoMessage() {}

func (x *ListBeerReply) ProtoReflect() protoreflect.Message {
	mi := &file_v1_catalog_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListBeerReply.ProtoReflect.Descriptor instead.
func (*ListBeerReply) Descriptor() ([]byte, []int) {
	return file_v1_catalog_proto_rawDescGZIP(), []int{1}
}

func (x *ListBeerReply) GetResults() []*ListBeerReply_Beer {
	if x != nil {
		return x.Results
	}
	return nil
}

type CreateBeerReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Name        string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Description string                 `protobuf:"bytes,2,opt,name=description,proto3" json:"description,omitempty"`
	Count       int64                  `protobuf:"varint,3,opt,name=count,proto3" json:"count,omitempty"`
	Image       []*CreateBeerReq_Image `protobuf:"bytes,4,rep,name=image,proto3" json:"image,omitempty"`
}

func (x *CreateBeerReq) Reset() {
	*x = CreateBeerReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_v1_catalog_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateBeerReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateBeerReq) ProtoMessage() {}

func (x *CreateBeerReq) ProtoReflect() protoreflect.Message {
	mi := &file_v1_catalog_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateBeerReq.ProtoReflect.Descriptor instead.
func (*CreateBeerReq) Descriptor() ([]byte, []int) {
	return file_v1_catalog_proto_rawDescGZIP(), []int{2}
}

func (x *CreateBeerReq) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *CreateBeerReq) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *CreateBeerReq) GetCount() int64 {
	if x != nil {
		return x.Count
	}
	return 0
}

func (x *CreateBeerReq) GetImage() []*CreateBeerReq_Image {
	if x != nil {
		return x.Image
	}
	return nil
}

type CreateBeerReply struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Id          int64                    `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	Name        string                   `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Description string                   `protobuf:"bytes,3,opt,name=description,proto3" json:"description,omitempty"`
	Count       int64                    `protobuf:"varint,4,opt,name=count,proto3" json:"count,omitempty"`
	Image       []*CreateBeerReply_Image `protobuf:"bytes,5,rep,name=image,proto3" json:"image,omitempty"`
}

func (x *CreateBeerReply) Reset() {
	*x = CreateBeerReply{}
	if protoimpl.UnsafeEnabled {
		mi := &file_v1_catalog_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateBeerReply) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateBeerReply) ProtoMessage() {}

func (x *CreateBeerReply) ProtoReflect() protoreflect.Message {
	mi := &file_v1_catalog_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateBeerReply.ProtoReflect.Descriptor instead.
func (*CreateBeerReply) Descriptor() ([]byte, []int) {
	return file_v1_catalog_proto_rawDescGZIP(), []int{3}
}

func (x *CreateBeerReply) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *CreateBeerReply) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *CreateBeerReply) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *CreateBeerReply) GetCount() int64 {
	if x != nil {
		return x.Count
	}
	return 0
}

func (x *CreateBeerReply) GetImage() []*CreateBeerReply_Image {
	if x != nil {
		return x.Image
	}
	return nil
}

type GetBeerReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Id int64 `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
}

func (x *GetBeerReq) Reset() {
	*x = GetBeerReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_v1_catalog_proto_msgTypes[4]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *GetBeerReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetBeerReq) ProtoMessage() {}

func (x *GetBeerReq) ProtoReflect() protoreflect.Message {
	mi := &file_v1_catalog_proto_msgTypes[4]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetBeerReq.ProtoReflect.Descriptor instead.
func (*GetBeerReq) Descriptor() ([]byte, []int) {
	return file_v1_catalog_proto_rawDescGZIP(), []int{4}
}

func (x *GetBeerReq) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

type GetBeerReply struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Id          int64                 `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	Name        string                `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Description string                `protobuf:"bytes,3,opt,name=description,proto3" json:"description,omitempty"`
	Count       int64                 `protobuf:"varint,4,opt,name=count,proto3" json:"count,omitempty"`
	Image       []*GetBeerReply_Image `protobuf:"bytes,5,rep,name=image,proto3" json:"image,omitempty"`
}

func (x *GetBeerReply) Reset() {
	*x = GetBeerReply{}
	if protoimpl.UnsafeEnabled {
		mi := &file_v1_catalog_proto_msgTypes[5]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *GetBeerReply) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetBeerReply) ProtoMessage() {}

func (x *GetBeerReply) ProtoReflect() protoreflect.Message {
	mi := &file_v1_catalog_proto_msgTypes[5]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetBeerReply.ProtoReflect.Descriptor instead.
func (*GetBeerReply) Descriptor() ([]byte, []int) {
	return file_v1_catalog_proto_rawDescGZIP(), []int{5}
}

func (x *GetBeerReply) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *GetBeerReply) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *GetBeerReply) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *GetBeerReply) GetCount() int64 {
	if x != nil {
		return x.Count
	}
	return 0
}

func (x *GetBeerReply) GetImage() []*GetBeerReply_Image {
	if x != nil {
		return x.Image
	}
	return nil
}

type UpdateBeerReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Id          int64                  `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	Name        string                 `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Description string                 `protobuf:"bytes,3,opt,name=description,proto3" json:"description,omitempty"`
	Count       int64                  `protobuf:"varint,4,opt,name=count,proto3" json:"count,omitempty"`
	Image       []*UpdateBeerReq_Image `protobuf:"bytes,5,rep,name=image,proto3" json:"image,omitempty"`
}

func (x *UpdateBeerReq) Reset() {
	*x = UpdateBeerReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_v1_catalog_proto_msgTypes[6]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateBeerReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateBeerReq) ProtoMessage() {}

func (x *UpdateBeerReq) ProtoReflect() protoreflect.Message {
	mi := &file_v1_catalog_proto_msgTypes[6]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateBeerReq.ProtoReflect.Descriptor instead.
func (*UpdateBeerReq) Descriptor() ([]byte, []int) {
	return file_v1_catalog_proto_rawDescGZIP(), []int{6}
}

func (x *UpdateBeerReq) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *UpdateBeerReq) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *UpdateBeerReq) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *UpdateBeerReq) GetCount() int64 {
	if x != nil {
		return x.Count
	}
	return 0
}

func (x *UpdateBeerReq) GetImage() []*UpdateBeerReq_Image {
	if x != nil {
		return x.Image
	}
	return nil
}

type UpdateBeerReply struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Id          int64                    `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	Name        string                   `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Description string                   `protobuf:"bytes,3,opt,name=description,proto3" json:"description,omitempty"`
	Count       int64                    `protobuf:"varint,4,opt,name=count,proto3" json:"count,omitempty"`
	Image       []*UpdateBeerReply_Image `protobuf:"bytes,5,rep,name=image,proto3" json:"image,omitempty"`
}

func (x *UpdateBeerReply) Reset() {
	*x = UpdateBeerReply{}
	if protoimpl.UnsafeEnabled {
		mi := &file_v1_catalog_proto_msgTypes[7]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateBeerReply) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateBeerReply) ProtoMessage() {}

func (x *UpdateBeerReply) ProtoReflect() protoreflect.Message {
	mi := &file_v1_catalog_proto_msgTypes[7]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateBeerReply.ProtoReflect.Descriptor instead.
func (*UpdateBeerReply) Descriptor() ([]byte, []int) {
	return file_v1_catalog_proto_rawDescGZIP(), []int{7}
}

func (x *UpdateBeerReply) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *UpdateBeerReply) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *UpdateBeerReply) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *UpdateBeerReply) GetCount() int64 {
	if x != nil {
		return x.Count
	}
	return 0
}

func (x *UpdateBeerReply) GetImage() []*UpdateBeerReply_Image {
	if x != nil {
		return x.Image
	}
	return nil
}

type DeleteBeerReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Id int64 `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
}

func (x *DeleteBeerReq) Reset() {
	*x = DeleteBeerReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_v1_catalog_proto_msgTypes[8]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *DeleteBeerReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteBeerReq) ProtoMessage() {}

func (x *DeleteBeerReq) ProtoReflect() protoreflect.Message {
	mi := &file_v1_catalog_proto_msgTypes[8]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteBeerReq.ProtoReflect.Descriptor instead.
func (*DeleteBeerReq) Descriptor() ([]byte, []int) {
	return file_v1_catalog_proto_rawDescGZIP(), []int{8}
}

func (x *DeleteBeerReq) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

type DeleteBeerReply struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
}

func (x *DeleteBeerReply) Reset() {
	*x = DeleteBeerReply{}
	if protoimpl.UnsafeEnabled {
		mi := &file_v1_catalog_proto_msgTypes[9]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *DeleteBeerReply) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteBeerReply) ProtoMessage() {}

func (x *DeleteBeerReply) ProtoReflect() protoreflect.Message {
	mi := &file_v1_catalog_proto_msgTypes[9]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteBeerReply.ProtoReflect.Descriptor instead.
func (*DeleteBeerReply) Descriptor() ([]byte, []int) {
	return file_v1_catalog_proto_rawDescGZIP(), []int{9}
}

type ListBeerReply_Beer struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Id          int64                       `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	Name        string                      `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Description string                      `protobuf:"bytes,3,opt,name=description,proto3" json:"description,omitempty"`
	Count       int64                       `protobuf:"varint,4,opt,name=count,proto3" json:"count,omitempty"`
	Image       []*ListBeerReply_Beer_Image `protobuf:"bytes,5,rep,name=image,proto3" json:"image,omitempty"`
}

func (x *ListBeerReply_Beer) Reset() {
	*x = ListBeerReply_Beer{}
	if protoimpl.UnsafeEnabled {
		mi := &file_v1_catalog_proto_msgTypes[10]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListBeerReply_Beer) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListBeerReply_Beer) ProtoMessage() {}

func (x *ListBeerReply_Beer) ProtoReflect() protoreflect.Message {
	mi := &file_v1_catalog_proto_msgTypes[10]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListBeerReply_Beer.ProtoReflect.Descriptor instead.
func (*ListBeerReply_Beer) Descriptor() ([]byte, []int) {
	return file_v1_catalog_proto_rawDescGZIP(), []int{1, 0}
}

func (x *ListBeerReply_Beer) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *ListBeerReply_Beer) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *ListBeerReply_Beer) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *ListBeerReply_Beer) GetCount() int64 {
	if x != nil {
		return x.Count
	}
	return 0
}

func (x *ListBeerReply_Beer) GetImage() []*ListBeerReply_Beer_Image {
	if x != nil {
		return x.Image
	}
	return nil
}

type ListBeerReply_Beer_Image struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Url string `protobuf:"bytes,1,opt,name=url,proto3" json:"url,omitempty"`
}

func (x *ListBeerReply_Beer_Image) Reset() {
	*x = ListBeerReply_Beer_Image{}
	if protoimpl.UnsafeEnabled {
		mi := &file_v1_catalog_proto_msgTypes[11]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListBeerReply_Beer_Image) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListBeerReply_Beer_Image) ProtoMessage() {}

func (x *ListBeerReply_Beer_Image) ProtoReflect() protoreflect.Message {
	mi := &file_v1_catalog_proto_msgTypes[11]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListBeerReply_Beer_Image.ProtoReflect.Descriptor instead.
func (*ListBeerReply_Beer_Image) Descriptor() ([]byte, []int) {
	return file_v1_catalog_proto_rawDescGZIP(), []int{1, 0, 0}
}

func (x *ListBeerReply_Beer_Image) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

type CreateBeerReq_Image struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Url string `protobuf:"bytes,1,opt,name=url,proto3" json:"url,omitempty"`
}

func (x *CreateBeerReq_Image) Reset() {
	*x = CreateBeerReq_Image{}
	if protoimpl.UnsafeEnabled {
		mi := &file_v1_catalog_proto_msgTypes[12]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateBeerReq_Image) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateBeerReq_Image) ProtoMessage() {}

func (x *CreateBeerReq_Image) ProtoReflect() protoreflect.Message {
	mi := &file_v1_catalog_proto_msgTypes[12]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateBeerReq_Image.ProtoReflect.Descriptor instead.
func (*CreateBeerReq_Image) Descriptor() ([]byte, []int) {
	return file_v1_catalog_proto_rawDescGZIP(), []int{2, 0}
}

func (x *CreateBeerReq_Image) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

type CreateBeerReply_Image struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Url string `protobuf:"bytes,1,opt,name=url,proto3" json:"url,omitempty"`
}

func (x *CreateBeerReply_Image) Reset() {
	*x = CreateBeerReply_Image{}
	if protoimpl.UnsafeEnabled {
		mi := &file_v1_catalog_proto_msgTypes[13]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateBeerReply_Image) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateBeerReply_Image) ProtoMessage() {}

func (x *CreateBeerReply_Image) ProtoReflect() protoreflect.Message {
	mi := &file_v1_catalog_proto_msgTypes[13]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateBeerReply_Image.ProtoReflect.Descriptor instead.
func (*CreateBeerReply_Image) Descriptor() ([]byte, []int) {
	return file_v1_catalog_proto_rawDescGZIP(), []int{3, 0}
}

func (x *CreateBeerReply_Image) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

type GetBeerReply_Image struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Url string `protobuf:"bytes,1,opt,name=url,proto3" json:"url,omitempty"`
}

func (x *GetBeerReply_Image) Reset() {
	*x = GetBeerReply_Image{}
	if protoimpl.UnsafeEnabled {
		mi := &file_v1_catalog_proto_msgTypes[14]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *GetBeerReply_Image) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetBeerReply_Image) ProtoMessage() {}

func (x *GetBeerReply_Image) ProtoReflect() protoreflect.Message {
	mi := &file_v1_catalog_proto_msgTypes[14]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetBeerReply_Image.ProtoReflect.Descriptor instead.
func (*GetBeerReply_Image) Descriptor() ([]byte, []int) {
	return file_v1_catalog_proto_rawDescGZIP(), []int{5, 0}
}

func (x *GetBeerReply_Image) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

type UpdateBeerReq_Image struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Url string `protobuf:"bytes,1,opt,name=url,proto3" json:"url,omitempty"`
}

func (x *UpdateBeerReq_Image) Reset() {
	*x = UpdateBeerReq_Image{}
	if protoimpl.UnsafeEnabled {
		mi := &file_v1_catalog_proto_msgTypes[15]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateBeerReq_Image) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateBeerReq_Image) ProtoMessage() {}

func (x *UpdateBeerReq_Image) ProtoReflect() protoreflect.Message {
	mi := &file_v1_catalog_proto_msgTypes[15]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateBeerReq_Image.ProtoReflect.Descriptor instead.
func (*UpdateBeerReq_Image) Descriptor() ([]byte, []int) {
	return file_v1_catalog_proto_rawDescGZIP(), []int{6, 0}
}

func (x *UpdateBeerReq_Image) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

type UpdateBeerReply_Image struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Url string `protobuf:"bytes,1,opt,name=url,proto3" json:"url,omitempty"`
}

func (x *UpdateBeerReply_Image) Reset() {
	*x = UpdateBeerReply_Image{}
	if protoimpl.UnsafeEnabled {
		mi := &file_v1_catalog_proto_msgTypes[16]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateBeerReply_Image) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateBeerReply_Image) ProtoMessage() {}

func (x *UpdateBeerReply_Image) ProtoReflect() protoreflect.Message {
	mi := &file_v1_catalog_proto_msgTypes[16]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateBeerReply_Image.ProtoReflect.Descriptor instead.
func (*UpdateBeerReply_Image) Descriptor() ([]byte, []int) {
	return file_v1_catalog_proto_rawDescGZIP(), []int{7, 0}
}

func (x *UpdateBeerReply_Image) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

var File_v1_catalog_proto protoreflect.FileDescriptor

var file_v1_catalog_proto_rawDesc = []byte{
	,
}

var (
	file_v1_catalog_proto_rawDescOnce sync.Once
	file_v1_catalog_proto_rawDescData = file_v1_catalog_proto_rawDesc
)

func file_v1_catalog_proto_rawDescGZIP() []byte {
	file_v1_catalog_proto_rawDescOnce.Do(func() {
		file_v1_catalog_proto_rawDescData = protoimpl.X.CompressGZIP(file_v1_catalog_proto_rawDescData)
	})
	return file_v1_catalog_proto_rawDescData
}

var file_v1_catalog_proto_msgTypes = make([]protoimpl.MessageInfo, 17)
var file_v1_catalog_proto_goTypes = []interface{}{
	(*ListBeerReq)(nil),              // 0: catalog.service.v1.ListBeerReq
	(*ListBeerReply)(nil),            // 1: catalog.service.v1.ListBeerReply
	(*CreateBeerReq)(nil),            // 2: catalog.service.v1.CreateBeerReq
	(*CreateBeerReply)(nil),          // 3: catalog.service.v1.CreateBeerReply
	(*GetBeerReq)(nil),               // 4: catalog.service.v1.GetBeerReq
	(*GetBeerReply)(nil),             // 5: catalog.service.v1.GetBeerReply
	(*UpdateBeerReq)(nil),            // 6: catalog.service.v1.UpdateBeerReq
	(*UpdateBeerReply)(nil),          // 7: catalog.service.v1.UpdateBeerReply
	(*DeleteBeerReq)(nil),            // 8: catalog.service.v1.DeleteBeerReq
	(*DeleteBeerReply)(nil),          // 9: catalog.service.v1.DeleteBeerReply
	(*ListBeerReply_Beer)(nil),       // 10: catalog.service.v1.ListBeerReply.Beer
	(*ListBeerReply_Beer_Image)(nil), // 11: catalog.service.v1.ListBeerReply.Beer.Image
	(*CreateBeerReq_Image)(nil),      // 12: catalog.service.v1.CreateBeerReq.Image
	(*CreateBeerReply_Image)(nil),    // 13: catalog.service.v1.CreateBeerReply.Image
	(*GetBeerReply_Image)(nil),       // 14: catalog.service.v1.GetBeerReply.Image
	(*UpdateBeerReq_Image)(nil),      // 15: catalog.service.v1.UpdateBeerReq.Image
	(*UpdateBeerReply_Image)(nil),    // 16: catalog.service.v1.UpdateBeerReply.Image
}
var file_v1_catalog_proto_depIdxs = []int32{
	10, // 0: catalog.service.v1.ListBeerReply.results:type_name -> catalog.service.v1.ListBeerReply.Beer
	12, // 1: catalog.service.v1.CreateBeerReq.image:type_name -> catalog.service.v1.CreateBeerReq.Image
	13, // 2: catalog.service.v1.CreateBeerReply.image:type_name -> catalog.service.v1.CreateBeerReply.Image
	14, // 3: catalog.service.v1.GetBeerReply.image:type_name -> catalog.service.v1.GetBeerReply.Image
	15, // 4: catalog.service.v1.UpdateBeerReq.image:type_name -> catalog.service.v1.UpdateBeerReq.Image
	16, // 5: catalog.service.v1.UpdateBeerReply.image:type_name -> catalog.service.v1.UpdateBeerReply.Image
	11, // 6: catalog.service.v1.ListBeerReply.Beer.image:type_name -> catalog.service.v1.ListBeerReply.Beer.Image
	0,  // 7: catalog.service.v1.Catalog.ListBeer:input_type -> catalog.service.v1.ListBeerReq
	2,  // 8: catalog.service.v1.Catalog.CreateBeer:input_type -> catalog.service.v1.CreateBeerReq
	4,  // 9: catalog.service.v1.Catalog.GetBeer:input_type -> catalog.service.v1.GetBeerReq
	6,  // 10: catalog.service.v1.Catalog.UpdateBeer:input_type -> catalog.service.v1.UpdateBeerReq
	8,  // 11: catalog.service.v1.Catalog.DeleteBeer:input_type -> catalog.service.v1.DeleteBeerReq
	1,  // 12: catalog.service.v1.Catalog.ListBeer:output_type -> catalog.service.v1.ListBeerReply
	3,  // 13: catalog.service.v1.Catalog.CreateBeer:output_type -> catalog.service.v1.CreateBeerReply
	5,  // 14: catalog.service.v1.Catalog.GetBeer:output_type -> catalog.service.v1.GetBeerReply
	7,  // 15: catalog.service.v1.Catalog.UpdateBeer:output_type -> catalog.service.v1.UpdateBeerReply
	9,  // 16: catalog.service.v1.Catalog.DeleteBeer:output_type -> catalog.service.v1.DeleteBeerReply
	12, // [12:17] is the sub-list for method output_type
	7,  // [7:12] is the sub-list for method input_type
	7,  // [7:7] is the sub-list for extension type_name
	7,  // [7:7] is the sub-list for extension extendee
	0,  // [0:7] is the sub-list for field type_name
}

func init() { file_v1_catalog_proto_init() }
func file_v1_catalog_proto_init() {
	if File_v1_catalog_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_v1_catalog_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListBeerReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_v1_catalog_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListBeerReply); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_v1_catalog_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateBeerReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_v1_catalog_proto_msgTypes[3].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateBeerReply); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_v1_catalog_proto_msgTypes[4].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*GetBeerReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_v1_catalog_proto_msgTypes[5].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*GetBeerReply); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_v1_catalog_proto_msgTypes[6].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UpdateBeerReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_v1_catalog_proto_msgTypes[7].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UpdateBeerReply); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_v1_catalog_proto_msgTypes[8].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*DeleteBeerReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_v1_catalog_proto_msgTypes[9].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*DeleteBeerReply); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_v1_catalog_proto_msgTypes[10].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListBeerReply_Beer); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_v1_catalog_proto_msgTypes[11].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListBeerReply_Beer_Image); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_v1_catalog_proto_msgTypes[12].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateBeerReq_Image); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_v1_catalog_proto_msgTypes[13].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateBeerReply_Image); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_v1_catalog_proto_msgTypes[14].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*GetBeerReply_Image); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_v1_catalog_proto_msgTypes[15].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UpdateBeerReq_Image); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_v1_catalog_proto_msgTypes[16].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UpdateBeerReply_Image); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_v1_catalog_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   17,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_v1_catalog_proto_goTypes,
		DependencyIndexes: file_v1_catalog_proto_depIdxs,
		MessageInfos:      file_v1_catalog_proto_msgTypes,
	}.Build()
	File_v1_catalog_proto = out.File
	file_v1_catalog_proto_rawDesc = nil
	file_v1_catalog_proto_goTypes = nil
	file_v1_catalog_proto_depIdxs = nil
}
